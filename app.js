document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadData = (key, def = null) => { const data = localStorage.getItem(key); return data ? JSON.parse(data) : def; };
    const handleImageUpload = (fileInput, previewEl, storageEl) => {
        const file = fileInput.files[0]; if (!file) return;
        const reader = new FileReader();
        reader.onload = e => { previewEl.src = e.target.result; previewEl.style.display = 'block'; if (storageEl) storageEl.value = e.target.result; };
    ...
    const renderSettings = () => {
        ...
        if (s.logo) {
            ...
            if (s.logo.startsWith('http')) { query('input[name="logo-source"][value="url"]').click(); companyLogoUrlInput.value = s.logo; } 
            else { query('input[name="logo-source"][value="local"]').click(); }
        }
    };
    ...
    const renderModalProducts = (filter = '') => {
        ...
        const filtered = products.filter(p => p.model.toLowerCase().includes(filter.toLowerCase()) || p.name.toLowerCase().includes(filter.toLowerCase()));
        if (filtered.length === 0) { modalProductList.innerHTML = '<tr><td colspan="4">没有找到匹配的商品</td></tr>'; return; }
        filtered.forEach(p => {
            const tr = document.createElement('tr');
            tr.dataset.id = p.id;
            tr.innerHTML = `<td><img src="${p.image || 'https://via.placeholder.com/50'}" alt="${p.name}"></td><td>${p.model}</td><td>${p.name}</td><td><button class="btn-primary btn-action btn-add-quote">添加</button></td>`;
    ...
    const renderQuoteItems = () => {
        ...
        quoteItems.forEach((item, index) => {
            ...
            tr.innerHTML = `
                ...
                <td><p><strong>${item.model}</strong>: ${item.name}</p><textarea class="quote-item-specs" rows="3">${item.specs}</textarea></td>
                <td><input type="number" class="quote-item-qty" value="${item.qty}" min="1"></td>
                <td><input type="text" class="quote-item-unit" value="${item.unit}"></td>
                <td><input type="number" class="quote-item-price" value="${price.toFixed(2)}" step="0.01"></td>
                <td class="quote-item-amount">${(item.qty * price).toFixed(2)}</td>
    ...
    const renderCurrencyOptions = () => {
        const currencies = settings.targetCurrencies || [{code: 'USD', rate: 7.25}];
        docCurrencySelect.innerHTML = currencies.map(c => `<option value="${c.code}">${c.code}</option>`).join('');
    };
    ...
    const calculatePrices = () => {
        ...
        if (isNaN(cost) || isNaN(margin) || currencies.length === 0) { productPricesDisplay.innerHTML = '请填写成本、利润率并在后台设置汇率。'; return {}; }
        let priceBase;
        if (algo === 'cost_plus') { priceBase = cost * (1 + margin / 100); } 
        else {
            if (margin >= 100) { productPricesDisplay.innerHTML = '<span style="color:red;">毛利率不能超100%</span>'; return {}; }
            priceBase = cost / (1 - margin / 100);
        }
        const prices = {};
        let pricesHTML = '<strong>销售价格:</strong><ul>';
        currencies.forEach(c => { const price = priceBase / c.rate; prices[c.code] = price; pricesHTML += `<li>${c.code}: ${price.toFixed(2)}</li>`; });
        pricesHTML += '</ul>';
    ...
    const handleNavClick = (e) => {
        ...
        if (targetId === 'pi-generator') {
            ...
            paymentTermsInput.value = s.defaultPayment || 'Example: 30% T/T deposit, 70% balance against B/L copy.';
            leadTimeInput.value = s.defaultLeadtime || 'Example: 25-30 days after receiving deposit.';
    ...
    const handleSettingsSave = (e) => {
        ...
        settings = {
            ...
            bankInfo: bankInfoInput.value, baseCurrency: baseCurrencyInput.value.trim().toUpperCase(), targetCurrencies: currencies,
            defaultPayment: defaultPaymentInput.value, defaultLeadtime: defaultLeadtimeInput.value, defaultPortLoading: defaultPortLoadingInput.value,
            defaultIncoterms: defaultIncotermsInput.value, defaultValidity: defaultValidityInput.value,
    ...
    const handleProductSave = (e) => {
        ...
        if (Object.keys(prices).length === 0) { alert('无法计算价格，请检查成本、利润率和后台汇率设置。'); return; }
        const imageSource = query('input[name="image-source"]:checked').value;
        const productData = {
            id: productIdInput.value || Date.now().toString(),
            model: productModelInput.value.trim(), name: productNameInput.value.trim(), specs: productSpecsInput.value.trim(),
            image: imageSource === 'local' ? productImageStorage.value : productImageUrlInput.value.trim(),
            unit: productUnitInput.value.trim(), hsCode: productHsCodeInput.value.trim(), packaging: productPackagingInput.value.trim(),
            netWeight: productNetWeightInput.value.trim(), grossWeight: productGrossWeightInput.value.trim(), prices: prices
        };
        if (!productData.model || !productData.name) { alert('请填写完整的产品型号和名称！'); return; }
        const existingIndex = products.findIndex(p => p.id === productData.id);
        if (existingIndex > -1) { products[existingIndex] = productData; } 
        else {
            if (products.some(p => p.model === productData.model)) { alert('产品型号已存在！'); return; }
    ...
    const handleProductListClick = (e) => {
        ...
        if (!product) { console.error('Data inconsistency error.', id); alert('出现数据错误，请刷新页面。'); return; }
        if (button.classList.contains('btn-edit')) {
            productIdInput.value = product.id; productModelInput.value = product.model; productNameInput.value = product.name;
            productSpecsInput.value = product.specs; productUnitInput.value = product.unit; productHsCodeInput.value = product.hsCode || '';
            productPackagingInput.value = product.packaging || ''; productNetWeightInput.value = product.netWeight || ''; productGrossWeightInput.value = product.grossWeight || '';
            ...
            productCostInput.value = ''; productMarginInput.value = '';
    ...
    const handlePiFormSubmit = (e) => {
        e.preventDefault();
        const currency = docCurrencySelect.value;
        const bankInfoHTML = docTypeSelect.value === 'PROFORMA INVOICE' && settings.bankInfo ? `<div class="print-terms"><h3>BANK INFORMATION:</h3><p>${settings.bankInfo.replace(/\n/g, '<br>')}</p></div>` : '';
        const remarksHTML = docRemarksInput.value ? `<div class="print-terms"><h3>REMARKS:</h3><p>${docRemarksInput.value.replace(/\n/g, '<br>')}</p></div>` : '';
        getEl('print-preview-area').innerHTML = `
            <div class="print-container">
                <header class="print-header">
                    <div class="seller-info"><h1>${settings.companyName||'Your Company'}</h1><p>${settings.companyAddress||'Your Address'}</p><p>${settings.companyContact||'Your Contact'}</p></div>
                    <div class="doc-title">${settings.logo ? `<img src="${settings.logo}" alt="logo">` : ''}<h2>${docTypeSelect.value.replace(/_/g,' ')}</h2></div>
                </header>
                <section class="print-meta">
                    <div class="buyer-info"><h3>TO:</h3><p><strong>${buyerNameInput.value}</strong></p><p>${buyerAddressInput.value.replace(/\n/g, '<br>')}</p><p>Attn: ${buyerAttnInput.value}</p></div>
                    <div class="doc-info">
                        <p><strong>No.:</strong> ${docIdInput.value}</p><p><strong>Date:</strong> ${docDateInput.value}</p>
                        <p><strong>Incoterms:</strong> ${docIncotermsInput.value}</p><p><strong>Validity:</strong> ${docValidityInput.value} Days</p>
                    </div>
                </section>
                <table class="print-table">
                    ...
                    <tbody>${quoteItems.map((item, index) => {
                        ...
                        const price = parseFloat(row.querySelector('.quote-item-price').value) || 0;
                        const qty = parseInt(row.querySelector('.quote-item-qty').value) || 0;
                        const specs = row.querySelector('.quote-item-specs').value.replace(/\n/g, '<br>');
                        return `<tr>
                            <td>${index + 1}</td><td><img src="${item.image || ''}"></td>
                            <td class="desc">
                                <p><strong>${item.model}:</strong> ${item.name}</p><p>${specs}</p>
                                ${item.hsCode ? `<p>HS Code: ${item.hsCode}</p>`:''}${item.packaging ? `<p>Packaging: ${item.packaging}</p>`:''}
                                ${item.netWeight ? `<p>N.W.:${item.netWeight}KGS / G.W.:${item.grossWeight}KGS</p>`:''}
                            </td>
                            <td>${qty}</td><td>${item.unit}</td><td>${price.toFixed(2)}</td><td>${(qty * price).toFixed(2)}</td>
                        </tr>`}).join('')}
                    </tbody>
                </table>
                <section class="print-summary">
                    <table>
                        <tr><td>Subtotal:</td><td>${subtotalSpan.textContent}</td></tr>
                        <tr><td>Freight Cost:</td><td>${parseFloat(freightCostInput.value || 0).toFixed(2)} ${currency}</td></tr>
                        <tr><td>Insurance:</td><td>${parseFloat(insuranceCostInput.value || 0).toFixed(2)} ${currency}</td></tr>
                        <tr class="grand-total"><td>GRAND TOTAL:</td><td>${grandTotalSpan.textContent}</td></tr>
                    </table>
                </section>
                <section class="print-terms">
                    <h3>TERMS & CONDITIONS:</h3>
                    <p><strong>Port of Loading:</strong> ${portLoadingInput.value}</p><p><strong>Port of Destination:</strong> ${portDestinationInput.value}</p>
                    <p><strong>Payment Terms:</strong> ${paymentTermsInput.value.replace(/\n/g, '<br>')}</p><p><strong>Lead Time:</strong> ${leadTimeInput.value.replace(/\n/g, '<br>')}</p>
                </section>
                ${remarksHTML}
                ${bankInfoHTML}
    ...
    const handleQuoteListUpdate = (e) => {
        ...
        if (e.target.classList.contains('btn-delete-quote-item')) {
            quoteItems.splice(parseInt(row.dataset.index, 10), 1);
            renderQuoteItems();
        } else if (e.target.matches('.quote-item-qty, .quote-item-price')) {
            updateTotals();
            const price = parseFloat(row.querySelector('.quote-item-price').value) || 0;
            const qty = parseInt(row.querySelector('.quote-item-qty').value) || 0;
            row.querySelector('.quote-item-amount').textContent = (qty * price).toFixed(2);
        }
    };
    ...
    function init() {
        ...
        const setupImageSourceToggle = (radioName, localDivId, urlDivId) => {
            queryAll(`input[name="${radioName}"]`).forEach(r => r.addEventListener('change', e => {
        ...
        [productCostInput, productMarginInput, ...queryAll('input[name="profit-algo"]')].forEach(el => el.addEventListener('input', calculatePrices));
        clearProductFormBtn.addEventListener('click', () => { productForm.reset(); productImagePreview.style.display = 'none'; productIdInput.value = ''; productPricesDisplay.innerHTML = ''; query('input[name="image-source"][value="local"]').click(); });
        productListBody.addEventListener('click', handleProductListClick);
        piForm.addEventListener('submit', handlePiFormSubmit);
        piForm.addEventListener('reset', () => { quoteItems = []; renderQuoteItems(); });
        generateDocIdBtn.addEventListener('click', generateDocId);
        docCurrencySelect.addEventListener('change', () => { quoteCurrencyLabel.textContent = docCurrencySelect.value; renderQuoteItems(); });
        showProductModalBtn.addEventListener('click', () => { renderModalProducts(); productSelectModal.style.display = 'block'; });
        closeModalBtn.addEventListener('click', () => productSelectModal.style.display = 'none');
        window.addEventListener('click', e => { if (e.target == productSelectModal) productSelectModal.style.display = 'none'; });
        modalSearchInput.addEventListener('input', e => renderModalProducts(e.target.value));
        modalProductList.addEventListener('click', e => {
            ...
            if (btn) {
                ...
                if (product) { quoteItems.push({ ...product, qty: 1 }); renderQuoteItems(); }
            ...
            if (file && confirm('导入数据将覆盖现有所有设置和商品，确定继续吗？')) {
                ...
                reader.onload = (event) => {
                    try {
                        ...
                        if (data.settings && data.products) { saveData('settings', data.settings); saveData('products', data.products); alert('数据导入成功！页面将重新加载。'); window.location.reload(); } 
                        else { alert('文件格式不正确！'); }
